...
function App() {
    ...
    return (
        ...
        >
            ...
            >
                <div className="m-8 absolute top-0 right-0 font-serif text-white text-[40px]">
                ...
                <div className="mx-8 my-6 absolute bottom-0 right-0 text-white text-[40px]">
                    {weatherData ? `${Math.round(weatherData.main.temp - 273.15)}°C` : "--°C"}
                </div>
            </div>

            {/* Right Side */}
            <WeatherProvider value={{ weatherData, search, isLoading }}>
                <div className="w-[25%] bg-black opacity-70 rounded-e-xl h-[600px] flex items-center justify-center">
                  <WeatherApp />
                </div>
            </WeatherProvider>
        </div>
